...
def tsunami_moment_func(x_lst, a, B, H):
    y_lst = []
    for i, x in enumerate(x_lst):
        if x*a < H:
...
with open('conditions.csv', 'r') as cfd, open('points_fall.csv', 'w+') as pfd:
        ...
        for i in range(2): # X or Y
            ...
            for k in range(3):
                for a in index_a: #culcurating tsunami force
                    ...
                    tsunami_moment = tsunami_moment_func(x, a, B+0.5, H)
                    ...
                    if(conditions[3]==1): #skelton case
                        ...
                        tsunami_moment = tsunami_moment*reduction_rate
                

                    #print(len(tsunami_force))
                    axis[k, i].plot(x, tsunami_moment,label=rf'a = {a}')            
                
                    ######################################################

                    if k  == 0:
                        idx = np.argwhere(np.diff(np.sign(tsunami_moment - arm*effective_weight_baloon))).flatten()
                    elif k == 1:
                        idx = np.argwhere(np.diff(np.sign(tsunami_moment - arm*effective_weight_normal))).flatten()
                    elif k==2:
                        idx = np.argwhere(np.diff(np.sign(tsunami_moment - arm*effective_weight_sunk))).flatten()
            ...
            axis[0, i].plot(x, arm*effective_weight_baloon,label="fall down limit$")
            axis[1, i].plot(x, arm*effective_weight_normal,label="fall down limit")
            axis[2, i].plot(x, arm*effective_weight_sunk,label="fall down limit")
